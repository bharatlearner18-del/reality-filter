...
<title>Premium — Reality Filter</title>
<meta name="description" content="Pick the launches and topics you care about — we re-check them and email you only when a verdict actually changes. Coming soon." />
...
<div class="topbar">
  <div class="wrap">
    <div class="nav-left">
      <a class="brand" href="index.html"><span class="brand-mark" aria-hidden="true"><svg viewBox="0 0 48 48" width="22" height="22"><circle cx="24" cy="24" r="18" fill="#d97757" fill-opacity=".12"/><path d="M24 6 A18 18 0 0 0 24 42 Z" fill="#d97757"/><circle cx="24" cy="24" r="18" fill="none" stroke="#d97757" stroke-width="2.6"/><g stroke="#c08532" stroke-width="2.6" stroke-linecap="round"><line x1="29" y1="18" x2="37" y2="18"/><line x1="29" y1="24" x2="39" y2="24"/><line x1="29" y1="30" x2="35" y2="30"/></g></svg></span> Reality Filter</a>
    </div>
    <nav class="nav-center">
      ...
      <a class="nav-cta" href="premium.html">Pricing</a>
        ...
        <svg class="ic-sun" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="4"></circle><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"></path></svg>
        <svg class="ic-moon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8z"></path></svg>
      </button>
    </nav>
  </div>
</div>
...
<main id="top">
  <section class="section section-premium" id="premium">
    <div class="wrap">
      ...
      <!-- .in-view in the markup so the band is visible even without JS — on this
           dedicated page the band IS the whole content, so it must not depend on the
           scroll-reveal script (which index.html needed only because the band sat
           below the fold). -->
      <div class="premium in-view">
        <div class="p-left">
          <span class="p-badge">Premium · coming soon</span>
          <h2 class="p-title">Stop checking back. We'll watch it for you.</h2>
          <p class="p-body">Pick the launches and topics you care about. We keep re-checking them for you and send one email the moment a verdict actually changes.</p>
          <ul class="p-list">
            <li><b>We track it for you.</b> Choose a company or topic once — our system re-checks it on every new signal and tells you the one thing that matters: hype, or reality.</li>
            <li><b>More checks in the search bar.</b> Ask about more launches, whenever you want — not just the occasional look.</li>
            <li><b>Deeper evidence.</b> The full data behind each verdict — benchmarks, sources and numbers, not just the headline call.</li>
          </ul>
        </div>
        <div class="waitlist">
          <p class="wl-lbl">Leave your email — we'll tell you when it's live</p>
          <form class="wlform" id="wlForm" autocomplete="off">
            <input type="email" id="wlEmail" required placeholder="you@work.com" aria-label="Your email" />
            <button class="btn" type="submit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.7 21a2 2 0 0 1-3.4 0"></path></svg>Notify me</button>
          </form>
          <p class="wl-done" id="wlDone">✓ You're on the list.</p>
        </div>
      </div>
    </div>
  </section>
...
<footer>
  <div class="wrap fwrap">
    <div>
      <div class="fbrand">Reality Filter</div>
      <p>The straight read on what's real in AI — backed by a public record of the calls we've made, and how they held up.</p>
    </div>
    <a class="toplink" href="index.html">← Back to check a launch</a>
  </div>
  <div class="wrap"><p class="disc">Verdicts are researched judgment calls, not buying advice. Verify anything that matters. Anything you send us is delivered through <a href="https://formspree.io/legal/privacy-policy" target="_blank" rel="noopener noreferrer">Formspree</a>.</p></div>
</footer>

<script src="forms.js?v=2"></script>
<script>
/* premium waitlist */
document.getElementById('wlForm').addEventListener('submit',async function(e){
  e.preventDefault();
  const email = document.getElementById('wlEmail').value.trim();
  if(!email) return;
  const b=this.querySelector('button');
  b.disabled=true; b.textContent="Sending…";
  const r = await sendSignal({type:'premium waitlist', email});
  if(!r.ok){ b.disabled=false; b.textContent="Notify me"; showFormError(this); return; }
  clearFormError(this);
  document.getElementById('wlDone').classList.add('show');
  this.querySelector('input').disabled=true;
  b.textContent="On the list ✓";b.style.opacity=.85;
});
</script>
<script>
// topbar shadow once the page scrolls
const tb = document.querySelector('.topbar');
addEventListener('scroll', ()=>tb.classList.toggle('scrolled', scrollY>8), {passive:true});
</script>
<script>
// theme toggle
document.querySelector('[data-theme-toggle]').addEventListener('click', ()=>{
  const dark = document.documentElement.classList.toggle('dark');
  try{ localStorage.setItem('rf-theme', dark?'dark':'light'); }catch(e){}
});
</script>
</body>
</html>
